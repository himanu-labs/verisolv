...
\Description{Architecture diagram with a top contract node labeled docs/CONTRACT.md connected by arrows to three pillar nodes below: Python solver (5 ODE, 2 PDE), Rust solver\_core (PyO3 kernel), and Lean 4 ODE (O(h) proof). Dashed edges mark the FFI boundary between Python and Rust and the shared-recurrence link between Python and Lean.}
...
\Description{Log-log plot of global error versus step size for explicit Euler and classical RK4 on the test problem y prime equals negative y. Two data series are shown with least-squares fitted lines: Euler with slope approximately 1 confirming first-order convergence, and RK4 with slope approximately 4 confirming fourth-order convergence. Dashed reference lines of slope 1 and slope 4 are overlaid.}
