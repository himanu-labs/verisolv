...
\documentclass[sigplan,nonacm,screen,review]{acmart}
...
\author{verisolv}
\affiliation{%
  \institution{Open-source research artifact}
  \country{}
}
\email{}
...
(BDF) stiff solver) and two finite-difference PDE solvers (1-D heat and wave),
...
and the Lean proof each conform to it independently.}
\label{fig:arch}
\end{figure}
...
\paragraph{BDF1/BDF2} is a simplified but genuine implicit stiff solver. It
...
with maturin as an \texttt{abi3} (stable application-binary-interface) wheel---one
...
identical to within $10^{-12}$, and in practice the maximum observed difference is
$0$.
...
The Lean development (\texttt{lean/ODE/EulerConvergence.lean}) proves that the
...
Table~\ref{tab:bench} reports our solvers against
\texttt{scipy.integrate.solve\_ivp} on four problems, measuring maximum error
...
variable-order BDF, though at higher cost---an expected consequence of fixed
